...
- **判定端点可配置**（`baseUrl`，默认 `https://api.typesafe.ai`）。此前端点是源码里的常量：key 属于
  自建或第三方 System One 主机（例如 Codiv）的人改不了它，请求一律发往 `api.typesafe.ai`——而 key 是
  **按主机签发**的，那边只会返回 401。因为每个能力都是 fail-open 的，401 的表现是剪枝、注入筛查、
  技能推荐**全部静默失效**，只在日志里留一条警告。现在这个值从设置（或 bundle 行的 `config:`）读到
  三个后端构造点，填裸主机名即可，`/v1/systemone` 仍由插件追加；端点与 key 一样按操作实时读取，
  改完不用重启。
